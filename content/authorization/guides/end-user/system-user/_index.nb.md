---
title: Systemtilgang veiledning for sluttbruker
linktitle: Systemtilgang
description: Her finner du enkle og trinnvise veiledninger som hjelper deg med å sette opp og bruke systemtilgang i Altinn.
toc: false
---

### Hva er systemtilgang?
Systemtilgang gir deg som sluttbruker mulighet til å bruke et fagsystem (for eksempel regnskap eller HR-system) til å hente og sende inn data til offentlige tjenester via Altinn – på vegne av din egen virksomhet eller for dine kunder. Systemtilgang kan brukes enten for din egen virksomhet eller for å utføre oppgaver på vegne av andre. Hvilken type du trenger, avhenger av om du rapporterer kun for deg selv – eller også for kunder, som for eksempel regnskapsfører.

Dette skjer ved hjelp av en systembruker, som er en teknisk bruker koblet til systemet. Systembrukeren har de nødvendige fullmaktene for å utføre oppgaver i Altinn, uten at du trenger å logge inn manuelt hver gang.

#### To typer systemtilgang:

- **Systemtilgang for egen virksomhet:** Brukes når du bare rapporterer for din egen virksomhet. Eksempel: sende inn A-melding eller MVA-melding fra et internt system.
- **Systemtilgang for kunder**: Brukes når du rapporterer på vegne av andre, som for eksempel regnskapsfører. Da må kundene gi deg nødvendige fullmakter via Altinn.

### Hvordan får du systemtilgang?
Systemtilgang kan opprettes på to måter:

- **Brukerstyrt opprettelse:** Du logger inn i Altinn og godkjenner systemtilgangen direkte.
- **Leverandørstyrt opprettelse:** Systemleverandøren sender en forespørsel som du godkjenner i Altinn.

Du må ha fullmakt til å gi systemet alle fullmaktene det ber om. Hvis du mangler én av dem, kan ikke systemtilgangen opprettes.

**Hva betyr dette i praksis?**
Når et fagsystem ber om systemtilgang via Altinn, må du godkjenne at systemet får fullmakt til bestemte tjenester – for eksempel å sende inn A-melding eller hente skattemeldinger.
Hver av disse tjenestene krever en fullmakt som kan følge av en bestemt rolle eller tilgangspakke i Altinn.
Hvis du ikke har alle fullmaktene som kreves for å gi systemet denne myndigheten, vil Altinn stoppe prosessen. Det holder ikke å ha noen av fullmaktene – du må ha alle som systemet ber om.

**Eksempel:**
Et fagsystem ber om fullmakt til både en tilgangspakke og en enkelttjeneste. Hvis du bare kan gi fullmakt til tilgangspakken, men ikke enkelttjenesten, kan du ikke godkjenne systemtilgangen. Du må enten få den manglende fullmakten eller sende forespørselen videre til en person som kan gi alle fullmaktene.

Velg guiden som passer for deg, og kom i gang med systemtilgang.

{{<children />}}
