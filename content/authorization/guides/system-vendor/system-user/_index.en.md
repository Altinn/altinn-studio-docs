...
4. Registering the System in Altinn’s System Register
   ...
   - Refer [here]({{< relref "/api/authentication/systemuserapi/systemregister/model/" >}}) for the detailed description for information about each input.
     ```json
     {
...
You will find additional details regarding all possible inputs, in the [system register model]({{< relref "/api/authentication/systemuserapi/systemregister/model/" >}}). For all subsequent communication, the system vendor must reference the value specified in the `id` field.
...
2. Vendor driven
   - SmartCloud AS creates a system user request (via altinn api for system user request) from within SmartCloud, which is sent to Altinn. For detailed information on each input field, refer the documentation [here]({{< relref "/api/authentication/systemuserapi/systemuserrequest/external/model/" >}})
     ```json
     {
   ...
   - The `confirmurl` is sent to the end user for approval. Follow the guide for end user for
     - [Standard system user]({{< relref "/authorization/guides/end-user/system-user/#guide-for-end-user-stadig-konsert-dagligleder-of-tilfeldig-subtil-ape" >}})
...
- The `confirmurl` is sent to the end user, here the auditing organisation for approval. Once approved, the end user can add clients to the system user. Follow the guide for end user for
  - [Agent system user]({{< relref "/authorization/guides/end-user/system-user/#guide-for-end-user-dress-minst-client-administrator-for-tilbakeholden-usymmetrisk-tiger-as-" >}})

See [Samarbeisportalen](https://samarbeid.digdir.no/altinn/systembruker/2542) for illustration and webinar.
{.mt-3}

Refer to [API documentation]({{< relref "/api/authentication/systemuserapi/" >}}) for more information on available endpoints.
