...
| `preselectedOptionIndex` | Index of the option to preselect (if no option has been selected yet) |  |
| `labelSettings` | See [Label settings](#label-settings-labelsettings). |  |
| `type` | Required. | `"RadioButtons"` |
...
| `layout` | Define the layout style for the options | `"column"`, `"row"`, `"table"` |
| `alertOnChange` | Boolean value indicating if the component should alert on change |  |
| `showLabelsInTable` | Boolean value indicating if the label should be visible when only one option exists in table |  |
| `showAsCard` | Boolean value indicating if the options should be displayed as cards. Defaults to false. |  |
...
| `label` |  |  |
| `label.dataType` | The name of the datamodel type to reference Required. |  |
| `label.field` | The path to the property using dot-notation Required. |  |
| `metadata` |  |  |
| `metadata.dataType` | The name of the datamodel type to reference Required. |  |
| `metadata.field` | The path to the property using dot-notation Required. |  |
