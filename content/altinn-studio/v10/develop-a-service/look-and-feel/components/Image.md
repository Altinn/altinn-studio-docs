...
| `image` | See [Image](#image-image). |  |

## Text resource bindings (`textResourceBindings`)

| Property | Description | Allowed Values |
|----------|-------------|----------------|
| `altTextImg` | Alternative text for the image (for screen readers). |  |
| `help` | Help text for the image (shown in help text tooltip/popup) |  |
...
|----------|-------------|----------------|
| `src` | Required. |  |
| `src.nb` | Image source (when using norwegian bokmål language) |  |
| `src.nn` | Image source (when using norwegian nynorsk language) |  |
| `src.en` | Image source (when using english language) |  |
| `width` | Image width Required. |  |
| `align` | Justification/alignment of the image Required. | `"flex-start"`, `"center"`, `"flex-end"`, `"space-between"`, `"space-around"`, `"space-evenly"` |
