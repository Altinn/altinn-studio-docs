...
| `type` | Required. | `"Accordion"` |
| `textResourceBindings` | See [Text resource bindings](#text-resource-bindings-textresourcebindings). |  |
| `children` | List of child component IDs to show inside the Accordion (limited to a few component types) Required. |  |
| `openByDefault` | Boolean value indicating if the accordion should be open by default |  |
