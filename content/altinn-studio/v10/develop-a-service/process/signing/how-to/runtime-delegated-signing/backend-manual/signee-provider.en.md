...
When an organization is provided as a signee, any person with a [key role](/en/altinn-studio/v10/reference/configuration/authorization/guidelines_authorization/roles_and_rights/roles_er) for the organization will be given access to `read` and `sign`.

The `Id` property in this implementation must match the ID specified in <altinn:signeeProviderId>.

{{% insert "content/altinn-studio/v10/develop-a-service/process/signing/how-to/runtime-delegated-signing/backend-manual/signee-provider-code.en.md" %}}
