...
| `type` | Required. | `"FileUploadWithTag"` |
...
| `maxFileSizeInMB` | Sets the maximum file size allowed in megabytes Required. |  |
| `maxNumberOfAttachments` | Sets the maximum number of attachments allowed to upload Required. |  |
| `minNumberOfAttachments` | Sets the minimum number of attachments required to upload Required. |  |
| `displayMode` | Required. | `"simple"`, `"list"` |
| `hasCustomFileEndings` | Boolean value indicating if the component has valid file endings |  |
| `validFileEndings` | A separated string of valid file endings to upload. If not set all endings are accepted. |  |
| `alertOnDelete` | Boolean value indicating if warning popup should be displayed when attempting to delete an element |  |
...
| `tagTitle` | The title to show when selecting a tag for each uploaded file |  |
...
| `list` | Required. |  |
| `list.dataType` | The name of the datamodel type to reference Required. |  |
| `list.field` | The path to the property using dot-notation Required. |  |
