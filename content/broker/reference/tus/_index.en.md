...
For smaller files and simple integrations, the [streaming upload](/broker/getting-started/developer-guides/send-files/#operation-uploadstreamed) endpoint remains the better choice.
...
See also [Large files](/broker/explanation/very-large-files/) for size limits and virus-scan options above 50 GB.
...
Before you upload, [initialize the file transfer](/broker/getting-started/developer-guides/send-files/#operation-initialize-filetransfer). Set file size, checksum, recipients, and other metadata in that call — the same as for a streaming upload.
...
4. **Wait for processing** — poll `GET /broker/api/v1/filetransfer/{fileTransferId}` or subscribe to [events](/broker/getting-started/developer-guides/events/) until status is `Published` (or `UploadProcessing` if virus scan is enabled).
...
- **Downloads** are not available via TUS. Recipients download files through the [standard download endpoint](/broker/getting-started/developer-guides/receive-files/).
...
- [Guide for sender — initialize and upload](/broker/getting-started/developer-guides/send-files/)
- [Large files](/broker/explanation/very-large-files/)
- [Events](/broker/getting-started/developer-guides/events/)
