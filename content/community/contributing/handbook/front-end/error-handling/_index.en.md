...
#### Actions
All actions should have sibling actions for _success_ and _error_. See [naming conventions](/en/community/contributing/handbook/naming#actions) for actions. The success action is triggered when everything is ok, while the error action is triggered if something goes wrong.

#### Sagas
If a saga needs to make an API call (or uses logic/utils that make API calls), this should be wrapped in a `try/catch` block. If an error occurs, this should be logged and the corresponding error action should be triggered. If no error occurs then the corresponding success action should be triggered. See [here](/en/community/contributing/handbook/front-end/redux/) for more information on actions and sagas.
