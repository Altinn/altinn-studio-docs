...
![Images settings anatomy](./images-settings-anatomy.png " Image settings")
...
### 2. Source
Property: `src`  
The source image can be external or [hosted in the app](#hosting-images-in-the-app).

For external images, the source is simply the image URL (e.g. `https://example.com/image.png`).  
See [Hosting images in the app](#hosting-images-in-the-app) for how to reference hosted images.  
...
Click the magnifying glass next to the property field to select an existing text resource or the `+` sign to create a new text.

### 4. Width
Property: `width`  
Image width is given as a percentage, with 100% as the original image size.
...
See [Horizontal alignment with `align`](#horizontal-alignment-with-align).

<br>

{{% expandlarge id="props-list" header="Properties List" %}}
