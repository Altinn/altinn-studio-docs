...
{{% anatomy-list %}}
1. **Komponent-ID** (`id`): Automatically generated component ID (editable).
2. **Kilde** (`src`): The [image source](#image-source).
3. **Alternativ tekst** (`textResourceBindings.altTextImg`): Alternative text. The text is stored as a [text resource](/app/development/ux/texts/#add-and-change-texts-in-an-application).
4. **Bredde** (`width`): Image width as a percentage, with 100% as the original image size.
5. **Plassering** (`align`): [Horizontal alignment of image](#horizontal-alignment-with-align).
{{% /anatomy-list %}}
...
```json{linenos=false,hl_lines="6-19"}
