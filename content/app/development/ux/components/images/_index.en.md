...
{{% notice info %}}
Static hosting is automatically configured for applications created after December 2021.
 For apps created before Dec 2021, static hosting must be [configured manually](#configure-static-hosting).
{{% /notice %}}
...
```json{linenos=false,hl_lines=["11", "22"]}
// File: /App/ui/layouts/<page>.json
...
Replace `applicationId` with the string value of `id` in `applicationmetadata.json`:
...
{{% expandlarge id="examples" header="Properties description and examples" %}}
