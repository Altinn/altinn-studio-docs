...
| data                  | A list of data elements. This include all forms, attachments and other data types being collected. See [DataElement](/en/api/models/data-element/)                                                                                                                  |
| presentationTexts     | A dictionary with text values extracted from forms that are saved on the instance. Values are extracted based on PresentationField values stored in the application metadata document. Also see [PresentationField](/en/api/models/app-metadata#presentationfield) |
| dataValues            | A dictionary with data values extracted from forms that are saved on the instance. Values are extracted based on configured data fields in the applicationmetadata document. Also see [DataField](/en/api/models/app-metadata#datafield)                           |
