...
The two `unauthorizedPresentation` values are enum values on the wire, so keep `Disabled` and
`Excluded` verbatim in code spans. Use *sperret* and *utelukket* only in running prose, and gloss
the value the first time a page uses it, e.g. `` `Excluded` `` (utelukket). Avoid *ekskludert*
unless a standalone label is needed - bare *utelukket* can read as "out of the question".
