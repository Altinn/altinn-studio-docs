...
Each authorization context sets an `unauthorizedPresentation`, either `Disabled` or `Redacted`, which decides what an unauthorized end user sees for that part of the dialog. Either way, the entity itself is never removed from the response - Dialogporten does not silently drop entities the current user cannot access, so list positions and counts stay stable regardless of who is asking.

With `Disabled`, the entity stays visible with its content intact, but any URLs are replaced with a placeholder. With `Redacted`, the entity is additionally stripped of its content, leaving only enough to show that something exists in that position without disclosing what it is.
