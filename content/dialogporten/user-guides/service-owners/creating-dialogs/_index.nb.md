...
Navigasjonshandlinger har aldri støttet `authorizationAttribute` - de kan bare ha en [autorisasjonskontekst]({{< relref "../../../getting-started/authorization/authorization-contexts/" >}}), som evalueres som en `read`-sjekk med mindre den selv navngir en annen `action`. Hvis navigasjonshandlingens egen kontekst gjør sluttbrukeren uautorisert, eller den overordnede forsendelsen selv er uautorisert, skrives URL-en om til `urn:dialogporten:unauthorized` (eller tittelen tømmes også, hvis `unauthorizedPresentation` er `Redacted`).
