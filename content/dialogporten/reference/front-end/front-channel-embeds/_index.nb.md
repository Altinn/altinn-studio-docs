...
: SBS brukes til å beskrive systemet sluttbrukeren bruker, vanligvis klientsiden av en nettleserapplikasjon som kjører JavaScript eller WebAssembly-kode.

Innholdsleverandørs system (ILS)
: Innholdsleverandørsystemet er systemet som svarer på endepunktene (URL-er) som FCE refererer til. Dette er vanligvis tjenesteleverandørplattformen, som Altinn 3.

## Grunnleggende trinn

1. Sluttbrukersystemet (SBS) identifiserer et bestemt innholdselement som en front channel embed
...
Se [innholdstyper](/nb/dialogporten/reference/front-end/front-channel-embeds/../../content-types/) for en liste over innholdsfelt der en FCE kan brukes som medietype. Denne listen kan endres, så SBS-implementatorer anbefales å håndtere FCE-medietyper på alle innholdsfelt.

FCE-medietypene kan identifiseres ved prefikset `application/vnd.dialogporten.frontchannelembed-url`. Alle FCE-er gir imidlertid også et eksternt innholdstype-suffiks, som indikerer hvilken type data endepunktet hos ILS kan forventes å returnere. For øyeblikket er to formater tillatt, for Markdown og HTML, noe som gir oss disse gyldige medietypene:
...
ILS MÅ validere det mottatte tokenet ved å sjekke tokensignaturen mot JWK-ene publisert av Dialogporten, og ved å sjekke at tokenet ikke er utløpt. Ytterligere valideringer KAN utføres, f.eks. kontroll av "actions"-claimet. ILS KAN validere `Origin`-headeren, men MÅ IKKE stole på denne alene for autorisasjonsformål, da den lett kan forfalskes.

Gitt vellykket autorisasjon og ingen andre feiltilstander, MÅ ILS svare med `200 OK` og en `Content-Type`-header som samsvarer med den eksterne innholdsdelen av medietypen for FCE, for øyeblikket enten `text/markdown` eller `text/html`, og en kropp som inneholder innholdet for FCE. Kroppen KAN være tom, og i så fall KAN ILS bruke svarkoden `204 No Content`.

I tilfelle feil oppstår, se [feilhåndtering](#feilhåndtering) nedenfor.

### Trinn 4. Håndtere svaret

SBS MÅ følge omdirigeringssvar (3xx), men MÅ også håndtere uendelige omdirigeringer ved å begrense antall omdirigeringer til minst 5. SBS MÅ sørge for at den returnerte `Content-Type`-headeren samsvarer med FCE-medietypen. FCE MÅ håndtere en tom responskropp, og i så fall skal ingen UI-endringer skje, slik at ILS kan bruke FCE-er som en måte å utløse eksterne prosesser på.

Gitt et vellykket svar, `200 OK`, og en ikke-tom kropp, bør SBS gjengi innholdet og injisere det i brukergrensesnittet etter eget skjønn.
...
ILS KAN levere en RFC9457 Problem Details-responskropp, og legge til detaljer om feiltilstanden som oppsto, inkludert et "details"-felt med menneskelig lesbar informasjon som SBS KAN vise til sluttbrukeren for støtteformål. Feildetaljene MÅ lokaliseres til språket som er angitt av innholdsfeltet.

SBS MÅ håndtere 5xx- og 4xx-svar ved å vise en generisk, lokalisert feilmelding, og KAN inkludere detaljer levert av ILS, hvis tilgjengelig. SBS BØR ta grep for å gjøre det klart for sluttbrukeren hvilken informasjon som ble gitt av ILS. Både SBS og ILS BØR logge feilen som oppsto for videre diagnose.
...
* Attributter som forårsaker skriptutførelse er ikke tillatt, MÅ IKKE produseres av ILS, og MÅ fjernes av SBS
* SBS BØR tydelig indikere at eventuelle lenker som peker til eksterne URL-er faktisk er eksterne, og legge til [rel="noreferrer"](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Attributes/rel/noreferrer)-attributter til ankertaggene
