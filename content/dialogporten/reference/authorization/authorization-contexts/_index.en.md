...
Must not start with `urn:altinn:resource:` - use `serviceResource` for that. Same format and length rules as `serviceResource`.
...
Type `string` enum, values `Disabled` or `Redacted`. **Required.**
...
- **`Disabled`** masks the URLs on the entity but keeps its other content visible. This is the behaviour equivalent to the legacy authorization attribute mechanism, and is the recommended default when migrating an existing dialog with no intent to change what end users see.
- **`Redacted`** additionally strips the entity's content, leaving a tombstone: the entity is still present at its position with minimal fields, but nothing about its actual content is revealed. See the next section for the exact effect per surface.

## What an unauthorized end user sees

Common to both presentations, on every surface: `isAuthorized` is `false`, `contextToken` is `null`, and the entity **remains present** in its array - authorization contexts never cause Dialogporten to remove an entity from a response.

Both presentations replace URLs with one of two placeholder values: `urn:dialogporten:unauthorized` for an access denial, and `urn:dialogporten:expired` for an expired attachment URL - but only when the caller is authorized; an unauthorized *and* expired attachment shows the unauthorized placeholder, not the expired one.

| Surface | `Disabled` | `Redacted` (additionally) |
|---|---|---|
| GUI action | `url` replaced with the placeholder. `title`, `prompt`, `priority`, `httpMethod`, `isDeleteDialogAction`, `action` and `id` are all kept. | `title` is emptied and `prompt` is cleared. `url` remains the placeholder; `id`, `action`, `priority`, `httpMethod` and `isDeleteDialogAction` survive. |
| API action | Every endpoint `url` is replaced with the placeholder; other endpoint fields (`version`, `httpMethod`, `documentationUrl`, `requestSchema`, `responseSchema`, `deprecated`, `sunsetAt`) are kept. `name` is kept. | `name` is cleared and `endpoints` is emptied entirely - no endpoint metadata at all. `id` and `action` survive. |
| Transmission | The content reference (used for [front channel embeds]({{< relref "/dialogporten/reference/front-end/front-channel-embeds" >}})) is replaced with the placeholder. `content.title` and `content.summary` remain readable. `sender`, `type`, `extendedType`, `externalReference`, `relatedTransmissionId`, `attachments` and `navigationalActions` all remain, with children masked individually - see below. | `sender`, `content`, `extendedType`, `externalReference`, `authorizationAttribute` and `relatedTransmissionId` are all cleared, and `attachments` and `navigationalActions` are emptied. Survivors: `id`, `createdAt`, `type`, `isOpened`, `isAuthorized`. |
| Dialog attachment | Every URL is replaced with the placeholder. `displayName`, `name` and `expiresAt` are kept. | `displayName` is emptied, `name` is cleared, and `urls` is emptied. Survivors: `id`, `expiresAt`. |
| Transmission attachment | Every URL is replaced with the placeholder. | `displayName` is emptied, `name` is cleared, and `urls` is emptied. Survivors: `id`, `expiresAt`. |
| Transmission navigational action | `url` is replaced with the placeholder. `title` and `expiresAt` are kept. | `title` is emptied and `url` remains the placeholder. Survivor: `expiresAt`. This entity has no `id` at all, on either presentation. |

Three consequences worth keeping in mind:

1. **`Redacted` is a tombstone, not a removal.** The entity keeps its position and, except for navigational actions, its `id` and timestamps, so an end-user system can render "there is something here you don't have access to" rather than nothing at all.
2. **A `Redacted` transmission's children are not processed individually - they're gone along with it.** If the transmission itself is redacted, its attachments and navigational actions are emptied wholesale rather than each being evaluated and redacted on their own terms. A child with its own `Redacted` context inside a transmission that is only `Disabled` (and denied), however, is redacted individually according to its own context.
...
- On API actions and GUI actions, `authorizationAttribute` must be absent when `authorizationContext` is present, and the top-level `action` field must be absent too - use `authorizationContext.action` instead. Conversely, `action` is required when there is no context.
...
3. **`unauthorizedPresentation` has no legacy equivalent and must be chosen explicitly.** Legacy behaviour (mask URLs, keep content) corresponds to `Disabled`. Migrating with `Redacted` instead is a visible change for end users.
...
| `unauthorizedPresentation` omitted | `'UnauthorizedPresentation' is required and must be either 'Disabled' or 'Redacted'.` |
