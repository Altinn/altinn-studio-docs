...
`contextToken` is present if, and only if, the entity has an authorization context *and* the current user is authorized for it. Otherwise it is `null` - including whenever `isAuthorized` is `false`.
